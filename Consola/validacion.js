document.addEventListener('DOMContentLoaded', () => {
  ...
  function limpiarErrores() {
    ['nombreUsuario', 'contrasena', 'confirmarContrasena', 'direccionPostal', 'comuna', 'telefono', 'paginaWeb', 'entradaAficion']
  ...
  formulario.addEventListener('submit', e => {
    ...
    if (!nombre) {
      mostrarError('nombreUsuario', 'Este campo es obligatorio'); valido = false;
    } else if (!usuarioRegex.test(nombre)) {
      mostrarError('nombreUsuario', 'Nombre inválido. Debe comenzar con letra, 5-10 caracteres, dígitos solo al final'); valido = false;
    }

    const pwd = document.getElementById('contrasena').value.trim();
    if (!pwd) {
      mostrarError('contrasena', 'Este campo es obligatorio'); valido = false;
    } else if (!contrasenaRegex.test(pwd)) {
      mostrarError('contrasena', 'La contraseña debe tener 3-6 caracteres y contener al menos una letra y un dígito'); valido = false;
    } else if (nombre && pwd.toLowerCase().includes(nombre.toLowerCase())) {
      mostrarError('contrasena', 'La contraseña no puede contener el nombre de usuario'); valido = false;
    }

    const pwdConf = document.getElementById('confirmarContrasena').value.trim();
    if (pwd && !pwdConf) {
      mostrarError('confirmarContrasena', 'Confirma la contraseña'); valido = false;
    ...
    if (!dir) { mostrarError('direccionPostal', 'Este campo es obligatorio'); valido = false; }

    const comuna = document.getElementById('comuna').value;
    if (!comuna) { mostrarError('comuna', 'Debes seleccionar una comuna'); valido = false; }

    const tel = document.getElementById('telefono').value.trim();
    if (!tel) {
      mostrarError('telefono', 'Este campo es obligatorio'); valido = false;
    } else if (!telefonoRegex.test(tel)) {
      mostrarError('telefono', 'Formato inválido (debe ser 569XXXXXXXX)'); valido = false;
    }

    const web = document.getElementById('paginaWeb').value.trim();
    if (web && !urlRegex.test(web)) {
      mostrarError('paginaWeb', 'URL inválida. Debe ser dominio (ej: best.aliexpress.com) o con http:// o https://'); valido = false;
    }
